...
START_URL = f"{BASE_URL}/ivanovo/catalog/"
DEFAULT_DELAY = 0.12
...
def is_citilab_catalog_url(url: str) -> bool:
    ...
    if parsed.netloc not in {"citilab.ru", "www.citilab.ru"}:
        return False
    return parsed.path.startswith("/ivanovo/catalog/")
...
def extract_links(html: str) -> list[str]:
    ...
    for raw in re.findall(r'https?://(?:www\.)?citilab\.ru/ivanovo/catalog/[^"\']+', html):
        href = normalize_url(raw)
        if href not in seen and is_citilab_catalog_url(href):
            seen.add(href)
            result.append(href)

    for raw in re.findall(r'/ivanovo/catalog/[^"\']+', html):
        href = normalize_url(raw)
        if href not in seen and is_citilab_catalog_url(href):
            seen.add(href)
            result.append(href)
...
def main() -> int:
    ...
    csv_path = outdir / "citilab_ivanovo.csv"
    xlsx_path = outdir / "citilab_ivanovo.xlsx"
