...
def is_bad_section_url(url):
    low = (url or "").lower()
    return any(part in low for part in BAD_SECTION_URL_PARTS)
...
def extract_category_links():
    ...
    for a in soup.select("a[href]"):
        ...
        links[href] = title or href.rsplit("/", 1)[-1]
...
def extract_title(soup):
    ...
    title = re.sub(rf"\s+в\s+{re.escape(CITY_NAME)}\s*$", "", title, flags=re.I)
    return normalize_space(title)


def extract_category_from_breadcrumbs(soup):
    crumbs = []
    for a in soup.select("nav a[href], .breadcrumb a[href], .breadcrumbs a[href]"):
        href = a.get("href", "")
        text = normalize_space(a.get_text(" ", strip=True))
        if "/catalog/" in href and "/catalog/item/" not in href and text:
            crumbs.append(text)
...
def parse_item_page(item_url, fallback_category):
    ...
    if not title or is_bad_name(title):
        return None

    category = extract_category_from_breadcrumbs(soup) or fallback_category or LAB_NAME
...
def collect_all():
    ...
    for category_url, category_name in category_links.items():
        try:
            first_soup = soupify(category_url)
            last_page = extract_last_page(first_soup)
        except Exception:
            continue
        ...
        for page in range(1, last_page + 1):
            page_url = category_url if page == 1 else canonical_page_url(category_url, page)
            try:
                page_rows, _ = parse_listing_page(page_url, category_name)
            except Exception:
                continue
