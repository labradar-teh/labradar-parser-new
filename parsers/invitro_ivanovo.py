...
def is_ivanovo_analysis_url(url: str) -> bool:
    parsed = urlparse(url)
    path = parsed.path.rstrip("/")
    if not is_invitro_url(url):
        return False
    return path.startswith("/analizes/for-doctors/ivanovo/")


def is_detail_url(url: str) -> bool:
    if not is_ivanovo_analysis_url(url):
        return False
    ...
    return bool(re.search(r"/analizes/for-doctors/ivanovo/\d+/\d+/?$", path + "/"))
...
def extract_links(html: str) -> list[str]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not is_ivanovo_analysis_url(href):
            continue
        if href in seen:
            continue
        seen.add(href)
    ...
    for raw in re.findall(r'https?://www\.invitro\.ru/analizes/for-doctors/ivanovo/[^"\']+', html):
        ...
        if href not in seen and is_ivanovo_analysis_url(href):
            seen.add(href)
            result.append(href)
...
def main() -> int:
    ...
    csv_path = outdir / "invitro_ivanovo.csv"
    xlsx_path = outdir / "invitro_ivanovo.xlsx"
